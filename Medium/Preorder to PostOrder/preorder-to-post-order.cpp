...
public:
    ...
    Node* cBST(int &i,int mini,int maxi,int pre[],int n){
        if(i==n || pre[i]>maxi ||pre[i]<mini){
            return NULL;
        }
        Node* root=newNode(pre[i]);
        i++;
        root->left=cBST(i,mini,root->data,pre,n);
        root->right=cBST(i,root->data,maxi,pre,n);
        return root;
    }
    Node* post_order(int pre[], int size)
    {
        int i=0;
        return cBST(i,INT_MIN,INT_MAX,pre,size);
    }
};
