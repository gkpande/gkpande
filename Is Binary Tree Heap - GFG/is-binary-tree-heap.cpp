...
/*struct Node {
  ...
  bool iscbt(struct Node* tree,int index,int cnt){
      ...
      return (index < cnt && iscbt(tree->left,2*index+1,cnt) && iscbt(tree->right,2*index+2,cnt));
  }
...
bool isMaxHeap(struct Node* tree){
    ...
    if(tree->right==NULL && tree->data>=tree->left->data){
        return true;
    }
...
class Solution {
  public:
    bool isHeap(struct Node* tree) {
        ...
        return iscbt(tree,0,cnt)&& isMaxHeap(tree);
